  ...
  <div class="row  row-cols-1 row-cols-md-2 g-4 p-5">
    <div v-for="(row, indexR) in rows" :key="indexR">
      <div class="card" @click="selectRow(row)" data-bs-toggle="modal" data-bs-target="#exampleModal">
        <img :src="getPicture(row.fajtaFoto)" class="card-img-top" alt="..." />
        <div class="card-body">
          <h5 class="card-title">{{row.fajta}}</h5>
          <p class="card-text">            
          </p>
        </div>
      </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">{{selectedRow.fajta}}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <p><strong>Leírása</strong>: {{selectedRow.fajtaLeiras}}</p>
        <p><strong>Gondozása</strong>: {{selectedRow.gondozas}}</p>
      </div>
    </div>
  </div>
</div>  
