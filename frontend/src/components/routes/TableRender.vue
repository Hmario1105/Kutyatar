          ...
          >
            ...
            <div>
              <input
                ...
                v-model="row[key]"
                v-if="
                  (status == 'update' || status == 'insert') &&
                  indexR == selectedRowIndex
                "
              />
            </div>
          </td>
...
export default {
  ...
  methods: {
    ...
    deleteOnclicked(index) {
      let answer = window.confirm("Valóban törölni akarod?");
      if (answer) {
        //törlés
    ...
    rovidites(szoveg, maxHossz) {
      if(szoveg != null){

        if (szoveg.length <= maxHossz || Number.isInteger(szoveg)) return szoveg;
        let rSzoveg = szoveg.toString().substr(0, maxHossz - 1) + "...";
        return rSzoveg;
      }
    },
  },
